...
docker run -d --rm --name marketplace-test-core-rabbitmq-job --network=marketplace-test-network --network-alias test-core.rabbitmq.com^
 --hostname test-core.rabbitmq.com -p 5672 -e RABBITMQ_DEFAULT_USER=testCoreUser -e RABBITMQ_DEFAULT_PASS=testCorePassword rabbitmq:alpine || goto :cleanRedis
...
docker run -d --rm --name marketplace-test-core-kafka-job --network=marketplace-test-network --network-alias test-core.kafka.com^
 ...
 -e KAFKA_LISTENER_SECURITY_PROTOCOL_MAP=PLAINTEXT:PLAINTEXT,PLAINTEXT_HOST:PLAINTEXT  -e KAFKA_OFFSETS_TOPIC_REPLICATION_FACTOR=1 -e KAFKA_BROKER_ID=1^
 confluentinc/cp-kafka || goto :cleanZookeeper
